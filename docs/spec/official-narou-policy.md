...
- `wget` による取得が失敗した場合（名前解決の失敗・HTTP エラーなど）、その作品についてはエラー内容（`wget` の出力を含む）をログに出して処理を打ち切り、`narou update` は残りの作品の更新を続ける
  - Why: `wget` のエラー出力が文字コード未確定のまま narou 本体の日本語メッセージに埋め込まれると、文字コード不一致の例外で `narou update` 全体が異常終了し、本来のエラー内容も隠れてしまっていたため
