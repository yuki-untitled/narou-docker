...
- `wget` はシェルを介さずに起動し、URL・リクエストヘッダーの値（ユーザー設定の User-Agent を含む）は、記号（`"` `$` バッククォートなど）を含んでいても書かれたとおりに `wget` へ渡す。取得した本文の文字コードの扱いは変えない
  - Why: シェル向けの文字列を組み立てて実行すると、値に含まれる記号がシェルに解釈され、コマンドが壊れたり意図しないコマンドが実行されたりする余地があるため
- `wget` コマンド自体が見つからない場合も、取得の失敗として扱う（その作品のエラーをログに出して、次の作品に進む）
